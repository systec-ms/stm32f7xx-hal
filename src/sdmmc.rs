...
use crate::gpio::gpioc::{PC10, PC11, PC12, PC6, PC7, PC8, PC9};
use crate::gpio::gpiod::{PD2, PD6, PD7};
// use crate::time::Hertz; // TODO
use crate::embedded_time::rate::Hertz;

use crate::gpio::{Alternate, AF10, AF11, AF12};
//use crate::gpio:::{AF7, AF8};
use crate::pac::{SDMMC1, SDMMC2};
use crate::rcc::{Clocks, Enable, Reset, APB2};
//use crate::rcc::rec::{ResetEnable, SdmmcClkSelGetter}; // TODO
...
pins! {
    ...
    SDMMC2:
        CLK: [PD6<Alternate<AF11>>]
        CMD: [PD7<Alternate<AF11>>]
        D0: [PB14<Alternate<AF10>>]
        D1: [PB15<Alternate<AF10>>]
        D2: [PB3<Alternate<AF10>>]
        D3: [PB4<Alternate<AF10>>]
...
pub trait SdmmcExt<SDMMC>: Sized {
    ...
    fn sdmmc<PINS>(self, apb: &mut APB2, _pins: PINS, clocks: &Clocks) -> Sdmmc<SDMMC>
    where
        PINS: Pins<SDMMC>;
    ...
    fn sdmmc_unchecked(self, apb: &mut APB2, bus_width: BusWidth, clocks: &Clocks) -> Sdmmc<SDMMC>;
}
...
impl<S> Sdmmc<S> {
    ...
    fn clk_div(ker_ck: Hertz, sdmmc_ck: u32) -> Result<(u8, Hertz), Error> {
        let clk_div = ker_ck.0 / sdmmc_ck - 2;
        match clk_div {
            0..=255 => {
                let clk = Hertz(ker_ck.0 / (clk_div + 2));
                Ok((clk_div as u8, clk))
            }
            _ => Err(Error::BadClock),
        }
    }
}
// TODO remove $Rec:ident (rec::$Rec -> SDMMCX), functionality should be inside SDMMCX (enable, disable, reset)
// Blocked by:
// https://github.com/stm32-rs/stm32-rs/issues/655
// https://github.com/stm32-rs/stm32f7xx-hal/pull/152
macro_rules! sdmmc {
    ($($SDMMCX:ident: ($sdmmcX:ident),)+) => {
        $(
            impl SdmmcExt<$SDMMCX> for $SDMMCX {
                // type Rec = rec::$Rec; TODO

                fn sdmmc<PINS>(self, apb: &mut APB2, _pins: PINS,
                               clocks: &Clocks) -> Sdmmc<$SDMMCX>
                where
                    PINS: Pins<$SDMMCX>,
                {
                    Sdmmc::$sdmmcX(self, apb, PINS::BUSWIDTH, clocks)
                }

                fn sdmmc_unchecked(self, apb: &mut APB2, bus_width: BusWidth,
                                   clocks: &Clocks) -> Sdmmc<$SDMMCX>
                {
                    Sdmmc::$sdmmcX(self, apb, bus_width, clocks)
                }
            }
            ...
            impl Sdmmc<$SDMMCX> {
                ...
                pub fn $sdmmcX(
                    ...
                    assert!(clocks.is_pll48clk_valid(),"PLL48CLK is not valid"); // TODO Sysclk and check if all mcus have the same requirments
                    let ker_ck = Hertz(48_000_000);
                ...
                pub fn init_card(&mut self, freq: impl Into<Hertz>) -> Result<(), Error> {
                    ...
                    let ocr = loop {
                        ...
                        let ocr: OCR = self.sdmmc.resp1.read().bits().into();
                        if !ocr.is_busy() {
                            // Power up done
                            break ocr;
                        }
                    ...
                    let cid = ((self.sdmmc.resp1.read().bits() as u128) << 96)
                        | ((self.sdmmc.resp2.read().bits() as u128) << 64)
                        | ((self.sdmmc.resp3.read().bits() as u128) << 32)
                        | self.sdmmc.resp4.read().bits() as u128;
                    card.cid = cid.into();

                    // Get RCA
                    self.cmd(Cmd::send_rel_addr())?;
                    card.rca = self.sdmmc.resp1.read().bits() >> 16;

                    // Get CSD
                    self.cmd(Cmd::send_csd(card.rca << 16))?;
                    let csd = ((self.sdmmc.resp1.read().bits() as u128) << 96)
                        | ((self.sdmmc.resp2.read().bits() as u128) << 64)
                        | ((self.sdmmc.resp3.read().bits() as u128) << 32)
                        | self.sdmmc.resp4.read().bits() as u128;
                    ...
                    if freq.0 <= 25_000_000 {
                        ...
                        self.clkcr_set_clkdiv(freq.0)?;
                    } else {
                        // Switch to max clock for SDR12
                        self.clkcr_set_clkdiv(25_000_000)?;
                    }
                ...
                fn get_scr(&self, card: &mut Card) -> Result<(), Error> {
                    ...
                    while {
                        ...
                        if status.rxfifoe().bit_is_clear() {
                            // FIFO not empty
                            scr[i] = self.sdmmc.fifo.read().bits();
                            i += 1;
                        }
                ...
                fn switch_signalling_mode(
                    ...
                    while {
                        ...
                        if sta_reg.rxfifohf().bit() {
                            for _ in 0..8 {
                                status[idx] = self.sdmmc.fifo.read().bits();
                                idx += 1;
                            }
                        }
